...
def clicked():
    ...
    generateCollage(chipName, filePath, x_images, y_images, picture_format, index_of_first_image)
    status["text"] = "Done!"

def generateCollage(chipName, filePath, x_images, y_images, picture_format, index_of_first_image):
    inspection_images = [Image.open(f) for f in fileLocations(x_images, y_images, filePath, index_of_first_image)]
    collage = generateBlankImage(inspection_images[0], x_images, y_images)
    ...
    collage.save(filePath + chipName + "." + picture_format)


def fileLocations(x_images, y_images, filePath, index_of_first_image):
    image_name_list = []
...
def generateBlankImage(firstImage, x_images, y_images):
